...
export const prepareEnvironment = async (cwd, mode) => {
    const files = [
    ...
    if (isWorkspace) {
        ...
        files.unshift(path.join(root, '.env'), path.join(root, `.env.${mode}`), path.join(root, '.env.local'), path.join(root, `.env.${mode}.local`));
    }
